...
///os.EOL for system compatibility for return characters \n or \r\n
const os = require('os');
...
function writeMessage(socketIndex, message){
  ...
  connection.socketPool[socketIndex].socket.write(os.EOL);
}
...
function directMessage(nicknameIndex, message){
  ...
  connection.socketPool[nicknameIndex].socket.write(os.EOL);
}
...
let nickname = {
  ...
  callback: function(socketIndex, text){
    let textArray = text.replace(os.EOL, '').split(' ');
    console.log(textArray);
    ///Check if nickname is currently in use
    let nickNameIndex = findNickname(text.replace(os.EOL, '').toUpperCase());
